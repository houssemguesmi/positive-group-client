...
const getBonusTree = async (userId) => {
  try {
    ...
    await Promise.all(
      level1Invitees.invitees.map(async (invitee) => {
        ...
        !responseInvitee &&
          User.findByIdAndUpdate(userId, { $pullAll: { invitees: [invitee] } });
      })
    );
      ...
      try {
        await Promise.all(
          response[`level${i}Invitees`].map(async (invitee) => {
            if (invitee != null) {
              let invitedUsers = (
                await User.findById(invitee.inviteeId).select({
                  invitees: 1,
                  _id: 0,
                })
              ).invitees;
              if (invitedUsers) {
                await Promise.all(
                  invitedUsers.map(async (inviteeId) => {
